...
AutoPlay.wantedAchievements = [
  82,
  89,
  108, // elder calm, 100 antimatter, halloween
  225,
  227,
  229,
  279,
  ...
  575,
  397,
]; // max cps, max buildings, ascend right
AutoPlay.kittens = [
  31, 32, 54, 108, 187, 320, 321, 322, 425, 442, 462, 494, 613, 766, 865,
];
AutoPlay.cursors = [0, 1, 2, 3, 4, 5, 6, 43, 82, 109, 188, 189, 660, 764, 873];
AutoPlay.maxBuildings = [
  826, 827, 828, 829, 830, 831, 832, 833, 834, 835, 836, 837, 838, 839, 858,
];
AutoPlay.butterBiscuits = [
  334, 335, 336, 337, 400, 477, 478, 479, 497, 659, 699, 767, 862,
];
AutoPlay.expensive = [
  38, 39, 40, 41, 42, 55, 56, 80, 81, 88, 89, 90, 104, 105, 106, 107, 120, 121,
  122, 123, 150, 151, 256, 257, 258, 259, 260, 261, 262, 263, 338, 339, 340,
  341, 342, 343, 350, 351, 352, 403, 404, 405, 406, 407, 444, 445, 446, 447,
  448, 453, 454, 455, 456, 457, 458, 464, 465, 466, 467, 468, 469, 498, 499,
  500, 501, 535, 536, 538, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574,
  575, 576, 577, 578, 579, 580, 581, 582, 583, 584, 585, 586, 587, 588, 607,
  608, 609, 615, 616, 617, 652, 653, 654, 655, 656, 657, 658, 678, 679, 680,
  681, 682, 721, 722, 723, 724, 807, 808, 809, 810, 811, 812, 813, 814, 815,
  816, 820, 821, 822, 823, 867, 868, 869, 870, 871, 872,
]; // most expensive cookies

//might need to align if bigger changes in cookie clicker
AutoPlay.prioUpgrades = [
  363,
  323, // legacy, dragon
  411,
  412,
  ...
  268,
  520,
  181, // permanent slots, season switcher,
  282,
  283,
  284,
  291,
  393,
  394,
]; // better golden cookies, kittens, synergies
...
AutoPlay.allSeasonUpgrades = AutoPlay.valentineUpgrades
  .concat(AutoPlay.christmasUpgrades)
  .concat(AutoPlay.easterUpgrades)
  .concat(AutoPlay.halloweenUpgrades);
...
AutoPlay.lumpRelatedAchievements = range(307, 320).concat([
...
AutoPlay.lateAchievements = [262, 491, 367].concat(
  AutoPlay.lumpRelatedAchievements
);
...
AutoPlay.run = function () {
  ...
  if (AutoPlay.hyperActive || AutoPlay.now >= AutoPlay.deadline) {
    // high activity phase
  ...
  if (
    Game.bakeryNameL.textContent.slice(0, AutoPlay.robotName.length) !=
    AutoPlay.robotName
  ) {
    Game.bakeryNameL.textContent =
      AutoPlay.robotName + Game.bakeryNameL.textContent;
  ...
  if (Game.cookies < goal / 10) {
    ...
    if (Game.BuildingsOwned == 0) {
      ...
      if (Math.round(Game.cookiesd) == goal)
        ...
        Game.ObjectsById[0].buy(130 + AutoPlay.runRightCount);
      else if (cookieDiff < -6000000)
        Game.ObjectsById[0].buy(90 + AutoPlay.runRightCount);
      else if (cookieDiff < -30000)
        Game.ObjectsById[0].buy(50 + AutoPlay.runRightCount);
      else if (cookieDiff < 0)
        Game.ObjectsById[0].buy(22 + AutoPlay.runRightCount);
...
AutoPlay.handleGoldenCookies = function () {
  // pop first golden cookie or reindeer
...
AutoPlay.cheatGoldenCookies = function () {
  ...
  if (AutoPlay.Config.CheatGolden == 1) {
    ...
    if (!AutoPlay.grindingCheat()) return; // only cheat in grinding
    ...
    var timeToNextLevel =
      (2 * 60 * 60 * 1000) / ((level - AutoPlay.cheatMax + 8) / 10);
...
AutoPlay.handleClicking = function () {
  ...
  if (
    Game.ascensionMode == 1 &&
    AutoPlay.endPhase() &&
    !Game.Achievements["True Neverclick"].won &&
    !Game.cookieClicks
  ) {
    return;
  }
...
AutoPlay.handleSavings = function () {
  ...
  if (AutoPlay.Config.SavingStrategy == 0) {
    // None
    AutoPlay.savingsGoal = 0;
    return;
  }
  if (AutoPlay.Config.SavingStrategy == 2) {
    // LUCKY
    AutoPlay.savingsGoal = Game.unbuffedCps * 60 * 100;
    AutoPlay.addActivity(
      "Saving to lucky (" + Beautify(AutoPlay.savingsGoal) + " cookies)"
    );
    return;
  }
  if (AutoPlay.Config.SavingStrategy == 3) {
    // LUCKY FRENZY
    AutoPlay.savingsGoal = Game.unbuffedCps * 60 * 100 * 7;
    AutoPlay.addActivity(
      "Saving to lucky frenzy (" + Beautify(AutoPlay.savingsGoal) + " cookies)"
    );
    return;
  }
  ...
  if (elapsedTime < 0) {
    AutoPlay.savingsGoal = 0;
    AutoPlay.addActivity(
      "Not saving for first " + startTime / 60 / 1000 + "+ minutes!"
    );
    return;
  }
  if (Game.UpgradesById[52].bought && Game.UpgradesById[53].bought) {
    ...
    AutoPlay.savingsGoal = 0;
    AutoPlay.addActivity(
      "Not saving until golden cookie upgrades are purchased."
    );
    return;
  }
  ...
  if (elapsedTime < targetTime) {
    ...
    AutoPlay.addActivity(
      "Saving to " +
        Beautify(AutoPlay.savingsGoal) +
        " cookies (" +
        (scaling * 100).toFixed(1) +
        "%)"
    );
  } else {
    AutoPlay.addActivity(
      "Saving to " + Beautify(AutoPlay.savingsGoal) + " cookies"
    );
  }
  if (AutoPlay.savingsGoal > Game.Objects["Cursor"].getPrice()) {
    // saving is too expensive
  ...
  if (fractionSaved < 0.8) {
    AutoPlay.savingsStart =
      AutoPlay.now - startTime - (targetTime * fractionSaved) / scaling; // fraction towards goal
  }
};
...
AutoPlay.bestBuy = function () {
  ...
  for (var u in CookieMonsterData.Upgrades) {
    if (u in overrides) {
      ...
      CookieMonsterData.Upgrades[u].pp =
        Math.max(
          Game.Upgrades[u].getPrice() -
            (Game.cookies + CookieMonsterData.Cache.WrinklersTotal),
          0
        ) /
          Game.cookiesPs +
  ...
  // sugar frenzy check
  if (
    AutoPlay.canUseLumps &&
    Game.Upgrades["Sugar frenzy"].unlocked &&
...
AutoPlay.avoidbuy = function (up) {
  //normally we do not buy 227, 71, ...
  switch (up.id) {
    case 71:
    case 73:
      return (
        Game.Achievements["Elder nap"].won &&
    ...
    case 84:
      return (
        Game.Upgrades["Elder Pledge"].bought ||
        Game.Achievements["Elder calm"].won
      ); // elder covenant
    case 227:
...
AutoPlay.handleBuildings = function () {
  ...
  if (
    ...
    // Rigidel is in slot 0, buy the cheapest
    var minIdx = 0,
      minPrice = Game.ObjectsById[minIdx].price;
    for (var i = Game.ObjectsById.length - 1; i >= 0; i--)
...
AutoPlay.handleSeasons = function () {
  ...
  if (
    ...
    elfGrandmas = Game.Objects["Grandma"].pics.filter(function (p) {
      return p.pic == "elfGrandma.png";
    });
  ...
  if (
    !Game.Upgrades["Season switcher"].bought ||
    Game.ascensionMode == 1 ||
    Game.seasonUses > 20
  )
    return;
...
AutoPlay.useLump = function () {
  // recursive call to handle many sugar lumps
  AutoPlay.canUseLumps = false;
...
AutoPlay.handleGrimoires = function () {
  if (Game.isMinigameReady(Game.Objects["Wizard tower"])) {
    ...
    if (AutoPlay.cpsMult > 100) {
      ...
      if (AutoPlay.canUseLumps && Game.lumps > 100) {
        g.lumpRefill.click();
      }
    }
  }
...
AutoPlay.handleGarden = function () {
  if (Game.isMinigameReady(Game.Objects["Farm"])) {
    ...
    if (
      !AutoPlay.canUseLumps &&
      AutoPlay.gardenReady(g) &&
      !AutoPlay.finished &&
      !AutoPlay.harvestPlant &&
      !AutoPlay.lumpRelatedAchievements.every(function (a) {
        return Game.AchievementsById[a].won;
      })
    ) {
...
AutoPlay.gardenReady = function (g) {
  // have all plants and all cookies
  return (
...
AutoPlay.findPlants = function (game, idx) {
  ...
  var couldPlant = 0;
  if (AutoPlay.plantList[idx] != 0) {
    // already used
    var oldPlant = AutoPlay.plantDependencies[AutoPlay.plantList[idx]][0];
    AutoPlay.addActivity(
      "Trying to get plant " +
        game.plants[oldPlant].name +
        " on sector " +
        AutoPlay.sectorText(idx) +
        "."
    );
    AutoPlay.plantCookies = false;
  ...
  var chkx = idx % 2 ? 0 : 5;
  var chky = idx > 1 ? 0 : 5;
  if (game.isTileUnlocked(chkx, chky)) {
    // only plant if the spot is big enough
    if (
      !AutoPlay.havePlant(game, "everdaisy") &&
      game.plants["elderwort"].unlocked &&
      game.plants["tidygrass"].unlocked
    ) {
    ...
    if (
      !AutoPlay.havePlant(game, "queenbeetLump") &&
  ...
  for (var i = 3; i < AutoPlay.plantDependencies.length; i++) {
    // plant normal plants
    var plant = AutoPlay.plantDependencies[i][0];
    if (
      !AutoPlay.havePlant(game, plant) &&
      game.plants[AutoPlay.plantDependencies[i][1]].unlocked &&
      game.plants[AutoPlay.plantDependencies[i][2]].unlocked
    ) {
      // want it
...
AutoPlay.planting = function (game) {
  if (!game.plants["meddleweed"].unlocked) {
    // wait for meddleweed
    AutoPlay.plantList = [0, 0, 0, 0];
  ...
  AutoPlay.switchSoil(
    game,
    0,
    AutoPlay.plantPending ? "fertilizer" : "woodchips"
  ); // want mutations
  if (Game.Objects["Farm"].level < 4) {
    var targets = [
      [AutoPlay.plantDependencies[AutoPlay.plantList[0]][1], 3, 2],
      [AutoPlay.plantDependencies[AutoPlay.plantList[0]][2], 3, 3],
    ];
    if (game.isTileUnlocked(3, 4))
      targets = targets.concat([
        [AutoPlay.plantDependencies[AutoPlay.plantList[0]][1], 3, 4],
      ]);
...
AutoPlay.plantSector = function (game, sector, plant1, plant2, plant0) {
  ...
  if (plant0 == "dummy") {
    ...
    for (var x = X; x < X + 3; x++)
      for (var y = Y; y < Y + 3; y++) AutoPlay.plantSeed(game, thePlant, x, y);
    return;
  }
...
AutoPlay.plantSeed = function (game, seed, whereX, whereY) {
  if (
    AutoPlay.cpsMult >
    1 + 10 * (AutoPlay.grindingCheat() + (AutoPlay.Config.CheatGolden > 1))
  ) {
  ...
  if (oldPlant != 0) {
    // slot is already planted, try to get rid of it
  ...
  if (
    game.plants[seed].cost * 60 * Game.cookiesPs >
    Game.cookies - AutoPlay.savingsGoal
  )
    return;
...
AutoPlay.seedCalendar = function (game, sector) {
  ...
  if (
    !Game.Upgrades["Ichor syrup"].unlocked &&
  ...
  if (
    !Game.Upgrades["Green yeast digestives"].unlocked &&
  ...
  if (
    !Game.Upgrades["Duketater cookies"].unlocked &&
  ...
  if (
    !Game.Upgrades["Elderwort biscuits"].unlocked &&
    game.plants["elderwort"].unlocked
  ) {
    AutoPlay.switchSoil(game, sector, "fertilizer");
  ...
  if (
    !Game.Upgrades["Bakeberry cookies"].unlocked &&
  ...
  if (
    !Game.Upgrades["Wheat slims"].unlocked &&
  ...
  if (
    !Game.Upgrades["Fern tea"].unlocked &&
  ...
  AutoPlay.switchSoil(
    game,
    sector,
    AutoPlay.plantPending ? "fertilizer" : "clay"
  );
  if (AutoPlay.poppingWrinklers && game.plants["wrinklegill"].unlocked)
...
AutoPlay.cleanSector = function (game, sector, plant0) {
  ...
  if (plant0 == "all") {
    for (var x = X; x < X + 3; x++)
      for (var y = Y; y < Y + 3; y++)
        if (x != X + 1 || y != Y + 1) {
          // we do not really need that if, do we?
          var tile = game.getTile(x, y);
          if (tile[0] >= 1 && game.plantsById[tile[0] - 1].unlocked)
            game.harvest(x, y);
        }
    return;
  }
      ...
      if (game.isTileUnlocked(x, y)) {
        ...
        if (tile[0]) {
          ...
          if (!plant.unlocked) {
            ...
            AutoPlay.addActivity(
              plant.name + " is still growing, do not disturb!"
            );
            ...
            if (
              game.plantsUnlockedN == game.plantsN &&
              tile[1] >= game.plantsById[tile[0] - 1].mature
            ) {
              // is mature
          ...
          if (
            AutoPlay.plantCookies &&
            tile[1] >= game.plantsById[tile[0] - 1].mature
          )
            if (
              !AutoPlay.plantsMissing ||
              !game.isTileUnlocked(x - (x % 3), y - (y % 3))
            )
...
AutoPlay.harvest = function (game, x, y) {
  ...
  if (AutoPlay.plantList[sector] == 1)
    AutoPlay.cleanSector(game, sector, "all");
};
...
AutoPlay.switchSoil = function (game, sector, which) {
  ...
  FireEvent(
    l("gardenSoil-" + Game.Objects["Farm"].minigame.soils[which].id),
    "click"
  );
};
...
AutoPlay.handleStockMarket = function () {
  ...
  if (Game.isMinigameReady(Game.Objects["Bank"]) && !AutoPlay.wantAscend) {
    ...
    if (
      !Game.AchievementsById[459].won &&
      market.getGoodMaxStock(market.goodsById[market.goodsById.length - 1]) >
        1000
    ) {
      // 500 of each
    ...
    if (!Game.Achievements["Debt evasion"].won && !AutoPlay.plantPending) {
      l("bankLoan2").click();
      setTimeout(function () {
        AutoPlay.doAscend("trying debt evasion");
      }, 30 * 1000); // wait 30 seconds
    }
    if (!AutoPlay.goodsList) {
      // need to init goodsList
      AutoPlay.goodsList = [];
      for (var g in market.goods) {
        let good = market.goods[g];
    ...
    for (var g in market.goods) {
      ...
      if (good.stock < maxStock) {
        // can buy more
        if (price - goodItem.delta > goodItem.min && price < goodItem.buyHigh) {
          // price is rising: buy
          if (goodItem.min < goodItem.buyLow) {
            // it is very cheap
            market.buyGood(good.id, 10000); // buy all
            goodItem.max = price;
          } else if (goodItem.min < goodItem.buyMedium) {
            // it is reasonable
            market.buyGood(good.id, (maxStock * 0.8 - good.stock) << 0); // buy 80%
            goodItem.max = price;
          } else if (goodItem.min < goodItem.buyHigh) {
            // it is affordable
      ...
      if (good.stock > 0) {
        ...
        if (price + goodItem.delta < goodItem.max && price > goodItem.sellLow) {
          // price is dropping: sell
          if (goodItem.max > goodItem.sellHigh) {
            market.sellGood(good.id, 10000); // it is very expensive, sell all
            goodItem.min = price;
          } else if (goodItem.max > goodItem.sellLow) {
            // it is reasonable
...
AutoPlay.nightAtStocks = function () {
  ...
  for (var g in market.goods) {
    ...
    let price = market.getGoodPrice(good);
    let goodItem = AutoPlay.goodsList[good.id];
    if (price < goodItem.buyHigh) {
      // it is affordable
      market.buyGood(good.id, 10000); // buy all
    }
    if (price > goodItem.sellLow) {
      // it is reasonable
...
AutoPlay.handleWrinklers = function () {
  ...
  doPop =
    doPop ||
    (Game.Upgrades["Unholy bait"].bought &&
      !Game.Achievements["Moistburster"].won);
  doPop =
    doPop ||
  ...
  if (doPop) {
    ...
    AutoPlay.addActivity(
      "Popping one wrinkler per two hours, last " +
        (((AutoPlay.now - AutoPlay.wrinklerTime) / 1000 / 60) >> 0) +
        " minutes ago."
    );
...
if (Game.bakeryName.slice(0, AutoPlay.robotName.length) == AutoPlay.robotName) {
  Game.bakeryName = Game.bakeryName.slice(
    AutoPlay.robotName.length,
    Game.bakeryName.length
  );
...
AutoPlay.handleSmallAchievements = function () {
  ...
  if (
    AutoPlay.endPhase() &&
    !Game.Achievements["Cheated cookies taste awful"].won
  )
  ...
  if (!Game.Achievements["Olden days"].won) {
    ...
    Game.ShowMenu(what);
    AutoPlay.info(
      'found the forgotten madeleine at the very bottom of the "Info" menu'
    );
  }
  if (
    !Game.Achievements["Cookie-dunker"].won &&
    Game.milkProgress > 1 &&
    Game.milkHd > 0.34
  ) {
  ...
  if (
    !Game.Achievements["No time like the present"].won &&
    Game.Has("Wrapping paper") &&
    !Game.hasBuff("Gifted out")
  ) {
...
AutoPlay.handleAscend = function () {
  ...
  if (Game.ascensionMode == 0 && Game.prestige == 0) AutoPlay.canContinue(); // update achievement goals
  if (Game.AchievementsById[AutoPlay.nextAchievement].won) {
    ...
    AutoPlay.doAscend(
      "have achievement: " +
        Game.AchievementsById[AutoPlay.nextAchievement].ddesc.replace(
          /<q>.*?<\/q>/gi,
          ""
        ) +
        " after " +
  ...
  if (
    Game.ascensionMode == 1 &&
    !AutoPlay.canContinue() &&
    !Game.AchievementsById[AutoPlay.nextAchievement].won
  ) {
    AutoPlay.doAscend("reborn mode did not work, retry.", 0);
    return;
  }
  if (AutoPlay.preNightMode() && AutoPlay.Config.NightMode > 0) return; //do not ascend right before the night
  if (
    AutoPlay.endPhase() &&
    !Game.Achievements["Endless cycle"].won &&
    !Game.ascensionMode &&
    Game.Upgrades["Sucralosia Inutilis"].bought &&
  ...
  if (
    Game.Upgrades["Permanent upgrade slot V"].bought &&
  ...
  var daysInRun = (AutoPlay.now - Game.startDate) / 1000 / 60 / 60 / 24;
  if (
    AutoPlay.nextAchievement == 463 &&
    daysInRun > 10 &&
    Game.Objects["Bank"].minigame.profit > daysInRun * 300000
  ) {
    ...
    if (!AutoPlay.wantAscend && daysInRun > 20)
      AutoPlay.addActivity(
        "Still " +
          Beautify(maxDaysInRun - daysInRun) +
  ...
  if (
    !Game.Upgrades["Lucky digit"].bought &&
    Game.heavenlyChips > 777 &&
    Game.ascendMeterLevel > 0 &&
    Game.ascendMeterLevel < 20 &&
  ...
  if (
    !Game.Upgrades["Lucky number"].bought &&
    Game.heavenlyChips > 77777 &&
    Game.ascendMeterLevel > 0 &&
    Game.ascendMeterLevel < 200 &&
    (Game.prestige + Game.ascendMeterLevel) % 1000 == 777
  ) {
  ...
  if (
    !Game.Upgrades["Season switcher"].bought &&
    AutoPlay.nextAchievement == 108 &&
    Game.ascendMeterLevel > 1111
  ) {
...
AutoPlay.canContinue = function () {
  ...
  if (!Game.Achievements["True Neverclick"].won && Game.cookieClicks == 0) {
    ...
    if (AutoPlay.neverclickWarn)
      Game.Prompt(
        '<h3>Attention</h3><div class="block">' +
  ...
  if (
    !Game.Achievements["Speed baking I"].won &&
    ...
    !Game.Achievements["Speed baking II"].won &&
    ...
    for (
      var i = 1;
      i < 3;
      i++ // threefold clicking speed
    )
      setTimeout(function () {
        Game.ClickCookie(0, Game.computedMouseCps);
      }, 60 * i);
  } else if (
    !Game.Achievements["Speed baking III"].won &&
...
AutoPlay.doAscend = function (str, log) {
  ...
  if (
    Game.wrinklers.some(function (w) {
      return w.close;
    })
  ) {
  ...
  if (
    Game.Upgrades["Chocolate egg"].unlocked &&
    !Game.Upgrades["Chocolate egg"].bought
  ) {
    if (Game.dragonLevel >= 9) {
      // setting first aura to earth shatterer
...
AutoPlay.grinding = function () {
  let grindingStart =
    AutoPlay.wantedAchievements[AutoPlay.wantedAchievements.length - 10];
...
AutoPlay.grindingCheat = function () {
  ...
  let cheatingStart =
    AutoPlay.wantedAchievements[AutoPlay.wantedAchievements.length - 8];
...
AutoPlay.checkAllAchievementsOK = function () {
  //We do not stop for one-year legacy
  for (var i in Game.Achievements) {
    ...
    if (
      ...
      me.pool != "dungeon" &&
      me.id != 367 &&
      !AutoPlay.lateAchievements.includes(me.id)
    ) {
      // missing achievement
      AutoPlay.setMainActivity(
        "Missing achievement #" +
  ...
  for (var i of AutoPlay.lateAchievements) {
    ...
    if (!me.won && me.pool != "dungeon" && me.id != 367) {
      // missing achievement
      AutoPlay.setMainActivity(
        "Missing achievement #" +
          me.id +
  ...
  for (var i in Game.Upgrades) {
    ...
    if (me.pool == "prestige" && !me.bought) {
      // we have not all prestige upgrades yet
      AutoPlay.nextAchievement = 99; // follow the white rabbit (from dungeons)
      AutoPlay.setMainActivity(
        "Prestige upgrade " + me.name + " is missing, waiting to buy it."
      );
  ...
  if (!Game.Achievements["So much to do so much to see"].won) {
    //wait until one-year legacy (367)
...
AutoPlay.buyHeavenlyUpgrades = function () {
  ...
  if (!Game.Achievements["Reincarnation"].won) {
    // for many ascends
...
AutoPlay.handleDragon = function () {
  ...
  if (Game.Upgrades["A crumbly egg"].unlocked) {
    if (
      ...
      if (
        Game.dragonLevel >= 5 &&
        Game.dragonLevel < Game.dragonLevels.length - 3
      )
        obj = Game.ObjectsById[Game.dragonLevel - 5];
      else if (Game.dragonLevel >= Game.dragonLevels.length - 3) obj = "buy150";
      Game.specialTab = "dragon";
      Game.UpgradeDragon();
      Game.ToggleSpecialMenu(0);
      if (obj == null) return;
      if (obj == "buy150") {
        // after sacrificing 50 or 200 of all
        // handle garden before buying (low cps)
        if (Game.Objects["Farm"].amount == 0) Game.Objects["Farm"].buy(1);
        AutoPlay.handleMinigames();
        for (var o of Game.ObjectsById) o.buy(150 - o.amount);
      } // after sacrificing 100, get 50 back immediately
      else obj.buy(50 - obj.amount);
    }
    AutoPlay.petDragon();
  }
  ...
  if (
    Game.dragonLevel >= 21 &&
    Game.lumps > 99 &&
    !AutoPlay.lumpHarvestAchievements.includes(AutoPlay.nextAchievement)
  )
    // keep dragons curve for lump harvest achievements
    wantedAura = 15; // radiant appetite
  if (Game.dragonAura != wantedAura) {
    Game.specialTab = "dragon";
    Game.SetDragonAura(wantedAura, 0);
    Game.ConfirmPrompt();
    Game.ToggleSpecialMenu(0);
  }
  if (
...
AutoPlay.checkDragon = function (building) {
  ...
  if (!Game.Achievements["Here be dragon"].won) return true; // don't limit when first fully training

  building = Game.Objects[building];

  // haven't sacrificed first 100, buy no more than 100
  if (Game.dragonLevel - 5 <= building.id) return building.amount < 100;
...
AutoPlay.LoadConfig = function () {
  try {
    if (window.localStorage.getItem(AutoPlay.ConfigPrefix) != null) {
      AutoPlay.Config = JSON.parse(
        window.localStorage.getItem(AutoPlay.ConfigPrefix)
      );
      // Check values
      var mod = false;
      for (var i in AutoPlay.ConfigDefault) {
        if (
          typeof AutoPlay.Config[i] === "undefined" ||
          AutoPlay.Config[i] < 0 ||
...
AutoPlay.ToggleConfig = function (config) {
  ...
  l(AutoPlay.ConfigPrefix + config).className = AutoPlay.Config[config]
    ? "option"
    : "option off";
};
...
AutoPlay.ToggleConfigUp = function (config) {
  ...
  l(AutoPlay.ConfigPrefix + config).innerHTML =
    AutoPlay.Disp.GetConfigDisplay(config);
...
AutoPlay.Disp.AddMenuPref = function () {
  ...
  var div = document.createElement("div");
  div.className =
    "title " + AutoPlay.Disp.colorTextPre + AutoPlay.Disp.colorBlue;
...
AutoPlay.setBotMode = function () {
  ...
  AutoPlay.info(
    "The bot has changed mode to " +
      AutoPlay.ConfigData.BotMode.label[AutoPlay.Config.BotMode]
  );
...
AutoPlay.debugLogging = function (s) {
  try {
    ...
    var toAdd =
      "#debug logging autoplay V" + AutoPlay.version + ": " + s + "\n";
    window.localStorage.setItem("autoplayLog", before + toAdd);
  } catch (e) {}
};

AutoPlay.status = function (print = true) {
  ...
  for (var o in Game.Objects) {
    ...
    if (print && myl)
      AutoPlay.info("" + myl + " sugar lumps missing for " + me.name + ".");
    lum += myl;
  }
  lum -= Game.lumps;
  if (lum < 0) lum = 0;
...
AutoPlay.launch = function () {
  ...
  if (
    AutoPlay.launchCount < 5 &&
    localStorageGet(Game.SaveTo) &&
    !Game.AchievementsById[0].won
  ) {
  ...
  l("versionNumber").innerHTML =
    "v. " + Game.version + " (Mod v." + AutoPlay.version + ") Discord: _trojan";
  l("versionNumber").innerHTML =
    "v. " +
    Game.version +
    " <span " +
  ...
  if (Game.version != AutoPlay.gameVersion)
    AutoPlay.info(
      "Warning: cookieBot is last tested with " +
